...
var _ErrUserAcces_instances, _ErrUserAcces_toMentionJid, _ConnectServer_waSocket, _ConnectServer_middlewares;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ConnectServerCounter = exports.ConnectServer = exports.ErrUserAcces = void 0;
const core_wapi_1 = require("core-wapi");
const logger_1 = require("./logger");
const events_1 = __importDefault(require("events"));
class ErrUserAcces {
    constructor() {
        _ErrUserAcces_instances.add(this);
    }
    ADMIN_ONLY() {
        return `*Hanya admin group yang dapat mengakses*`;
    }
    SUPER_ADMIN_ONLY() {
        return `*Hanya pemilik group yang dapat mengakses*`;
    }
    SYS_CREATOR() {
        return `*Hanya pengarang asli bot yang dapat mengakses*`;
    }
    SYS_DEV_ONLY() {
        return `*Hanya pengembang bot yang dapat mengakses*`;
    }
    SYS_OWNER_ONLY() {
        return `*Hanya pemilik bot yang dapat mengakses*`;
    }
    SYS_CONTRIBUTOR_ONLY() {
        return `*Hanya kontributor bot yang dapat mengakses*`;
    }
    USER_ONLY() {
        return `*Hanya pengguna reguler yang dapat mengakses*`;
    }
    SPECIAL_USER_ONLY() {
        return `*Hanya pengguna istimewa yang dapat mengakses*`;
    }
    SELECTED_USER_ONLY() {
        return `*Hanya pengguna terpilih yang dapat mengakses*`;
    }
    VERIFIED_USER_ONLY() {
        return `*Hanya pengguna terverifikasi yang dapat mengakses*`;
    }
    REGISTERED_USER_ONLY() {
        return `*Hanya pengguna teregistrasi yang dapat mengakses*`;
    }
    BLOCKED_USER_ONLY() {
        return `*Hanya pengguna terblokir yang dapat mengakses*`;
    }
    NOT_ADMIN(user) {
        return `*Anda ${__classPrivateFieldGet(this, _ErrUserAcces_instances, "m", _ErrUserAcces_toMentionJid).call(this, user)} bukan admin*`;
    }
    SYS_NOT_ADMIN() {
        return `*System bot bukan admin*`;
    }
    static get(type, user) {
        return new this()[type](user);
    }
}
exports.ErrUserAcces = ErrUserAcces;
_ErrUserAcces_instances = new WeakSet(), _ErrUserAcces_toMentionJid = function _ErrUserAcces_toMentionJid(jid) {
        ...
        const eventsProcess = async (events) => {
            ...
            for (const eventType in eventsHandler) {
                ...
                const context = class Context {
                    ...
                    get isMeAddedToGroup() {
                        const m = (this.getInfo() || {})?.messages?.[0] || {};
                        return (m.messageStubType === 'GROUP_CREATE' || m.messageStubType === 20) && (m.participant && m.participant !== this.me);
                    }
                    get isMeJoinToGroup() {
                        const m = (this.getInfo() || {})?.messages?.[0] || {};
                        return (m.messageStubType === 'GROUP_CREATE' || m.messageStubType === 20) && !m.participant;
                    }
                };
                if (eventListen) {
                    try {
