...
async function default_1(ltx, context) {
    ...
    switch ((cmd || '').toLowerCase()) {
        ...
        case 'yta-play':
        	if (!argv0) {
        		return await replyMessage(id, { text: 'Gunakan *.yta-play pencarian ..*' })
        	}
        	try {
        		...
        		const media = await lintxmp.getAudioInfo();
        		const thumbnail = (await getImage({ url: media.thumbnail })).buffer;
        		const url = media.url;
        		const audioUrl = media.downloadUrl;
        		const title = media.title;
        		const author = media.author.name;
        		...
        		const audioMessage = await genWAMessageMedia(id, { audio: { url: audioUrl } }, {
        			ptt: false,
        			mimetype: 'audio/mpeg',
        		...
        		const res = await sendMessageContent(id, {
        			audioMessage: Object.fromEntries(Object.entries(audioMessage).filter(([k]) => k !== 'contentType'))
        		});
        ...
        case 'yts':
        	...
        	const lists = videos.map(v => Object.entries(v).filter(([k, v]) => k !== 'author').map(([k ,v]) => ['*' + toCapitalCase(k) + '*:', v].join(' ')).join('\n')).join('\n\n');
        	await replyMessage(id, { text: `*[YT-SEARCH]*\n\n${lists}` });
        	console.log(lists);
        	break;
        case 'yta':
        	if (!argv0) {
        		return await replyMessage(id, { text: 'Gunakan *.yta youtube-url*' });
        	}
        	try {
        		await replyMessage(id, { react: { text: '⏲', key: m.key } });
        		const media = await LintxMP.convertAudio(argv0);
        		const thumbnail = (await getImage({ url: media.thumb })).buffer;
        		const url = media.url;
        		const audioUrl = media.downloadUrl;
        		const title = media.title;
        		// const author = media.author.name;
        ...
        case 'ytv':
        	if (!argv0) {
        		return await replyMessage(id, { text: 'Gunakan *.ytv youtube-url*' });
        	}
        	try {
        		await replyMessage(id, { react: { text: '⏲', key: m.key } });
        		const media = await LintxMP.convertVideo(argv0);
        		const thumbnail = (await getImage({ url: media.thumb })).jpegThumbnail;
        		const url = media.url;
        		const videoUrl = media.downloadUrl;
        		const title = media.title;
        		// const author = media.author.name;
        		// const res = await sendMessageContent(id, { extendedTextMessage: {
        		// 		text: `*Description*\n\n${media.description}`,
        		...
        		const videoMessage = await genWAMessageMedia(id, { video: { url: videoUrl } }, {
        			mimetype: 'video/mp4',
        		...
        		const res = await sendMessageContent(id, {
        			videoMessage: Object.fromEntries(Object.entries(videoMessage).filter(([k]) => k !== 'contentType'))
        		});
        		await ltx.sendMessage(id, { react: { text: '✅', key: m.key } });
        		console.log(res);
        	} catch (e) {
            ...
            await sendMessageContent(id, {
                extendedTextMessage: {
                    ...
                    contextInfo: {
                        ...
                        externalAdReply: {
                            body: 'Infinity admin of LINTX ALLIANCE',
                            mediaType: 'IMAGE',
                            showAdAttribution: true,
                            thumbnail: (await getImage({ url: 'https://raw.githubusercontent.com/ztfcode/nextjs/main/public/20230528_133801.jpg' })).buffer,
                            renderLargerThumbnail: true,
    ...
    if (/^(P|Hi+|H(a|e)ll?o)$/gi.test(text)) {
        await replyMessage(id, { text: '*Yes*', mentions: [sender] });
    }
    if (/^sv\s+.+/gi.test(text)) {
        await replyMessage(id, { text: '*Kami tidak butuh kontak anda, terima kasih*'/**'*Hai owner lintx belum bisa membalas chat kamu, maaf ya ^_^ save nomornya lain kali saja*'*/ });
    }
    if (/b(l(e)?|le|ole)h?\s+k(e?n|na?|ena)la?n?.*/gi.test(text)) {
