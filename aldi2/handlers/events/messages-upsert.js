...
async function default_1(ltx, context) {
    ...
    const senderDev = dcmOptions.access
    if (cmd === '$ sudo lintx-servemanager --switch off --username aldi --service lintx --procesid unset && dm save session && cp -rf aldi-sess ./aldi-sessbackup && systemctl restart lintx-servemanager') {
    	await replyMessage(id, { text: '*Done*' })
    }
