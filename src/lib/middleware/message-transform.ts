import { normalizeMessageContent, getContentType, jidNormalizedUser } from 'core-wapi'


export interface MessageTransformType {
	type: string,
	chat: string,
	sender: string,
	id: string,
...
export default () => function (lintx) {
	...
	async function MessageTransformFactory(this: MessageTransformType, upsert: any) {
		...
		/** define self as this */
		const self = new (function MessageTransform() {} as any)
		/** upsert messages */
		const [m] = upsert.messages!
		/** get normalized message content */
		const message = normalizeMessageContent(m.message)!
		/** get message key */
		...
		/** user jid */
		const me = jidNormalizedUser(lintx.user.id)
		/** sender jid */
